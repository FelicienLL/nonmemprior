...
get_t_var <- function(theta_prec, type, theta_val = NULL){
  if(!type %in% c("RSE", "SE", "CI95")) stop("Type of precision on theta value is not supported")
  if(type %in% c("RSE", "CI95") & is.null(theta_val)) stop("Please provide theta values to compute variance")
  if(type == "RSE"){
